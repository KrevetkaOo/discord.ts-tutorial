  ...
  constructor() {
    super({
      ...
      async run({ client, int }) {
        ...
        if (!data) data = new Servidores({ _id: int.guildId });

        const cmd = int.options.getSubcommand(false);
        ...
        const commands = {
          ...
          channel: async () => {
            const channel = int.options.getChannel('target');
          ...
          config: async () => {
            ...
            const option = int.options.getString('action');
            const value = int.options.getBoolean('value');
